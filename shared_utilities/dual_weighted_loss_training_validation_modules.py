...
from metrics import *
from cg_ega_loss_metric import *
...
def train(train_iter, model, optimizer, lr_scheduler, config, ap_weight, be_weight, ep_weight, hypo_multiplier, train_loader, val_loader, save_dir):
    ...
    if best_model_state_dict:
        best_model_path = os.path.join(save_dir, f"best_validation_model_rmse_{best_rmse:.4f}.pth")
        torch.save(best_model_state_dict, best_model_path)
        print(f"Best model saved at {best_model_path} with RMSE: {best_rmse:.6f}")
    else:
